...
function fakeFileList(count: number): DocumenyType[] {
  const fileList = [];
  for (let i = 0; i < count; i += 1) {
    fileList.push({
      id: `fake-file-${i}`,
      ...
      status: ['Pending', 'Approved', 'Rejected'][i % 3] as
        ...
        | 'Approved'
        | 'Pending',
      documentType:['Confidential','Normal'] [i%2]  as 
      | 'Confidential'
      | 'Normal',
      viewedAt: new Date(new Date().getTime() - 1000 * 60 * 60 * 2 * i).getTime(),
      createdAt: new Date(new Date().getTime() - 1000 * 60 * 60 * 2 * i).getTime(),
...
let sourceFileData: DocumenyType[] = [];
...
function getFiles(req: Request, res: Response) {
  ...
  const result = fakeFileList(count);
  sourceFileData = result
  return res.json({
    data: {
      list: result,
    },
